...
co-abundance, locations g1 ... ,locations g2 ..., co locations, edge string (g1,g2)]
"""



class MouseDataset(Dataset):
    def __init__(self, data: pd.DataFrame, y_data: pd.DataFrame = None, expression_size: int = 2, prot_size: int = 3, locations_size: int = 40):
        data = copy.deepcopy(data)
        data['g1'] = [e.split(', ')[0].split('(')[-1] for e in data.iloc[:, -1]]
        data['g2'] = [e.split(', ')[-1].split(')')[0] for e in data.iloc[:, -2]]
        interaction = data.iloc[:, [expression_size + prot_size - 1, expression_size + prot_size + locations_size - 1]].astype(float).values
        ...
        graph_matrix_g1 = data.iloc[:, [0, expression_size] + list(range(expression_size + prot_size, int(expression_size + prot_size + locations_size / 2))) + [-2]]
        graph_matrix_g1 = graph_matrix_g1.rename(columns={'g1': 'g'})
        graph_matrix_g2 = data.iloc[:, [1, expression_size + 1] + list(range(int(expression_size + prot_size + locations_size / 2), int(expression_size + prot_size + locations_size - 1))) + [-1]]
        ...
        if y_data is not None:
          self._y_data = torch.tensor(y_data, dtype=torch.float32).to(DEVICE)
        else:
          self._y_data =None
