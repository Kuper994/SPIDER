...
class MouseDataset(Dataset):
    def __init__(self, data: pd.DataFrame, y_data: pd.DataFrame = None, expression_size: int = 2, prot_size: int = 3,
                 locations_size: int = 40):
        data = copy.deepcopy(data)
        data['g1'] = [e.split(', ')[0].split('(')[-1] for e in data.iloc[:, -1]]
        data['g2'] = [e.split(', ')[-1].split(')')[0] for e in data.iloc[:, -2]]
        interaction = data.iloc[:, [expression_size + prot_size - 1,
                                    expression_size + prot_size + locations_size - 1]].astype(float).values
